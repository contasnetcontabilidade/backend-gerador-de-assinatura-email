...
exports.exportRouter = exports.templateRouter = exports.usersRouter = void 0;
...
const controler_1 = __importDefault(require("../controller/controler"));
const upload = (0, multer_1.default)({
    storage: multer_1.default.memoryStorage(),
    limits: { fileSize: 30 * 1024 * 1024 }, // 30MB
});
exports.usersRouter = express_2.default.Router();
exports.usersRouter.post("/register", controler_1.default.createUser);
exports.usersRouter.get("/:_id", controler_1.default.getAllUsersOrByName);
exports.usersRouter.get("/", controler_1.default.getAllUsersOrByName);
exports.usersRouter.put("/update/:_id", controler_1.default.updateUser);
exports.usersRouter.delete("/delete/:_id", controler_1.default.deleteUser);
exports.usersRouter.post("/login", controler_1.default.loginUser);
exports.templateRouter = express_2.default.Router();
exports.templateRouter.post("/create", upload.fields([
    { name: "backgroundImg", maxCount: 1 },
    { name: "logoContas", maxCount: 1 },
]), controler_1.default.createTemplate);
exports.templateRouter.get("/", controler_1.default.getAllTemplatesOrById);
exports.templateRouter.get("/:_id", controler_1.default.getAllTemplatesOrById);
exports.templateRouter.put("/update/:_id", upload.fields([
    { name: "backgroundImg", maxCount: 1 },
    { name: "logoContas", maxCount: 1 },
]), controler_1.default.updateTemplate);
exports.templateRouter.delete("/delete/:_id", controler_1.default.deleteTemplate);
...
const defaultLogoDataUri = (() => {
    for (const logoPath of DEFAULT_LOGO_PATHS) {
        if (fs_1.default.existsSync(logoPath)) {
            const base64 = fs_1.default.readFileSync(logoPath).toString("base64");
...
const toGradient = (value, fallback) => {
    if (!value)
        return `linear-gradient(${fallback}, ${fallback})`;
...
const extractColors = (value) => {
    if (!value)
        return [DEFAULT_BLUE, DEFAULT_GREEN];
    const matches = value.match(/(#[0-9a-fA-F]{3,8}|rgba?\([^)]+\))/g);
    if (matches && matches.length > 0)
        return matches;
    return [value];
};
const buildStopList = (colors) => {
...
const buildHtml = (payload) => {
    ...
    const name = escapeHtml((_a = payload.name) !== null && _a !== void 0 ? _a : "");
    const email = escapeHtml((_b = payload.email) !== null && _b !== void 0 ? _b : "");
    const sector = escapeHtml((_c = payload.sector) !== null && _c !== void 0 ? _c : "");
    const userId = escapeHtml((_d = payload.userId) !== null && _d !== void 0 ? _d : "");
    ...
    const nameColor = (_e = payload.collaboratorNameColor) !== null && _e !== void 0 ? _e : "#000000";
    const sectorColor = (_f = payload.sectorNameColor) !== null && _f !== void 0 ? _f : "#000000";
    const iconColors = extractColors(payload.iconsColor);
    const iconStops = buildStopList(iconColors);
    const iconStart = (_g = iconColors[0]) !== null && _g !== void 0 ? _g : DEFAULT_BLUE;
    const iconEnd = (_h = iconColors[1]) !== null && _h !== void 0 ? _h : iconStart;
    ...
    const infoColor = (_k = (_j = payload.collaboratorInfosColor) !== null && _j !== void 0 ? _j : iconStart) !== null && _k !== void 0 ? _k : "#ffffff";
    const photo = payload.image
        ? `url('${payload.image}')`
        : "linear-gradient(120deg, #222, #444)";
    const imageZoom = Math.max((_l = payload.imageZoom) !== null && _l !== void 0 ? _l : 150, 50);
    const offsetX = (_m = payload.imageOffsetX) !== null && _m !== void 0 ? _m : 50;
    const offsetY = (_o = payload.imageOffsetY) !== null && _o !== void 0 ? _o : 50;
    const logoSrc = payload.contasLogo || defaultLogoDataUri;
    const logoImg = logoSrc ? `<img src="${logoSrc}" alt="Logo" />` : "";
    const titleBlock = payload.showTitle === false
        ? ""
...
exports.exportRouter.post("/signature", upload.single("image"), (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const body = req.body;
    ...
    try {
        ...
        yield page.setContent(html, { waitUntil: "networkidle0", timeout: 20000 });
        const element = yield page.$("#signature-preview");
        if (!element) {
            console.warn("Elemento de preview não encontrado");
            const buffer = yield page.screenshot({ type: "png", fullPage: true });
            res.contentType("image/png").send(buffer);
            return;
        }
        const buffer = yield element.screenshot({ type: "png", omitBackground: true });
        res.contentType("image/png").send(buffer);
    }
    catch (error) {
